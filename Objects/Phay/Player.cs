...
public partial class Player : CharacterBody2D
{
	...
	[Export] public float CoyoteTime = 0.3f;
	...
	private List<EntityInteractionHitbox> Interactables = [];
	...
	{
		var player = Game.PackedScenes.Player.Instantiate<Player>();
	...
	{
		if (!IsOnFloor()) return;
		if (!IsValidRespawnLocation()) return;
		if (Position.DistanceSquaredTo(RespawnPosition) < 100f) return;
	...
	{
		if (IsUpHeld)
			Game.Level.Camera.PanUp(CameraPanYOffset);
		else if (IsDownHeld)
			Game.Level.Camera.PanDown(CameraPanYOffset);
		else
			Game.Level.Camera.PanReset();
	}
}
